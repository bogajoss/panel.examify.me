...
function mapQuestionToPhpStyle(q: Question) {
  return {
    ...
    explanation_image_url: q.explanationImageId ? getImageUrl(q.explanationImageId) : "",
    type: q.type,
    section: q.section,
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1. Validate Token
    const secretToken = process.env.API_SECRET_TOKEN || "antigravity_secret_2026";
    ...
    switch (route) {
      case "files": {
        ...
        const files = (response.documents as unknown as QuestionFile[]).map(mapFileToPhpStyle);
        return NextResponse.json(files);
      }

      case "questions": {
        const fileId = searchParams.get("file_id");
        const queries: string[] = [Query.orderAsc("orderIndex"), Query.limit(500)];
        ...
        const questions = (response.documents as unknown as Question[]).map(mapQuestionToPhpStyle);
      ...
      case "question": {
        ...
        try {
          ...
          return NextResponse.json(mapQuestionToPhpStyle(question as unknown as Question));
        } catch (error) {
      ...
      default:
        return NextResponse.json({ error: "Route not found or not specified" }, { status: 404 });
    }
  } catch (error: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (route !== "update-question") {
      return NextResponse.json({ error: "Invalid route for POST" }, { status: 400 });
    }
    ...
    if (input.question_text !== undefined) updateData.questionText = input.question_text;
